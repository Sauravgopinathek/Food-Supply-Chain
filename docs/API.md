...
# API Reference — Food Supply Chain (on‑chain)

This document is a concise reference for the on‑chain contract `FoodSupplyChain` (source: `contracts/Traceability.sol`). It lists roles, events and the main functions the frontend and scripts use.

## Roles
- `DEFAULT_ADMIN_ROLE` — administrator (can grant roles, emergency actions)
- `HANDLER_ROLE` — creators/handlers, allowed to create shipments
- `CARRIER_ROLE` — carriers (transporters)
- `STORE_ROLE` — retailers / stores (receive shipments)
- `SENSOR_ROLE` — authorized sensors that can submit readings

## Events
- `TraceLog(uint256 indexed shipmentId, address indexed actor, uint256 timestamp, string action, string note, int256 temperature)` — emitted for creates, updates, transfers, contamination and sensor reports
- `ReputationUpdated(address indexed account, int256 newScore)` — emitted when a reputation score changes

## Core functions (summary)
- `createShipment(string product, string meta) returns (uint256)`
    - Creates a new shipment. Caller must have or become a handler. Returns the new shipment id.

- `addTrace(uint256 id, string location, int256 temperature, string note)`
    - Adds a trace record. Callable by the current holder or any account with `SENSOR_ROLE`. If `temperature` exceeds the safe threshold the shipment is marked contaminated and reputation adjustments occur.

- `transferCustody(uint256 id, address newHolder, string note)`
    - Transfer custody from the current holder to `newHolder`. Status will update to `TRANSIT` or `RECEIVED` depending on `newHolder`'s role. Reputation is adjusted when a handoff results in `RECEIVED` and the shipment is not contaminated.

- `getShipmentInfo(uint256 id) view returns (Shipment)` — returns the shipment struct with fields (id, createdAt, handler, contaminated, status, currentHolder)
- `getShipmentCount() view returns (uint256)` — total shipments created
- `isShipmentContaminated(uint256 id) view returns (bool)` — contamination state
- `submitSensorReadings(uint256 id, int256[] temps, string[] locations, uint256[] timestamps)` — bulk sensor reporting (only `SENSOR_ROLE`)
- `getReputation(address account) view returns (int256)` — returns reputation score for an address
- `grantMultipleRoles(address account, bytes32[] roles)` — admin helper to grant multiple roles
- `emergencyContaminateShipment(uint256 id, string reason)` — admin-only emergency contamination and penalty

For exact argument types and event shapes consult `contracts/Traceability.sol` and the generated artifact ABI under `artifacts/contracts/Traceability.sol/`.
