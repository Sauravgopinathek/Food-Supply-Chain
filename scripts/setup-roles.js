...
/**
 * FoodTrace Role Setup Script
 *
 * This script sets up roles for supply chain participants after deployment.
 * It should be run after the main deployment script for FoodTrace.
 */
...
async function main() {
    ...
    // Connect to the deployed contract (renamed contract)
    const FoodSupplyChain = await ethers.getContractFactory("FoodSupplyChain");
    const traceability = FoodSupplyChain.attach(contractAddress);
    ...
    // Define role constants (mapped to new role names)
    const PROCESSOR_ROLE = ethers.keccak256(ethers.toUtf8Bytes("HANDLER_ROLE"));
    const DISTRIBUTOR_ROLE = ethers.keccak256(ethers.toUtf8Bytes("CARRIER_ROLE"));
    const RETAILER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("STORE_ROLE"));
    const ORACLE_ROLE = ethers.keccak256(ethers.toUtf8Bytes("SENSOR_ROLE"));

    console.log("\n--- Available Roles (mapped) ---");
    console.log("HANDLER_ROLE (as PROCESSOR_ROLE):", PROCESSOR_ROLE);
    console.log("CARRIER_ROLE (as DISTRIBUTOR_ROLE):", DISTRIBUTOR_ROLE);
    console.log("STORE_ROLE (as RETAILER_ROLE):", RETAILER_ROLE);
    console.log("SENSOR_ROLE (as ORACLE_ROLE):", ORACLE_ROLE);
    ...
    console.log("Admin has HANDLER_ROLE:", adminHasProcessor ? "✅" : "❌");
