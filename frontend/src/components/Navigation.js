...
import { useRole, ROLE_LABELS } from '../utils/RoleContext';
 ...
 */
const Navigation = () => {
  const { account, networkInfo, contract, connectToWallet, disconnect, isLoading } = useBlockchain();
  ...
  return (
    <Navbar bg="dark" variant="dark" expand="lg" sticky="top">
      <Container>
        <Navbar.Brand href="/">
          <strong>Food supply chain</strong>
        </Navbar.Brand>
          ...
          {account ? (
            <>
              <Nav className="me-auto">
                ...
                <LinkContainer to="/rate-dealer">
                  <Nav.Link>Rate Dealer</Nav.Link>
                </LinkContainer>
              ...
              <Nav className="ms-auto">
                ...
                <Navbar.Text className="me-3 d-flex align-items-center">
                  <Badge bg={contract ? 'success' : 'secondary'} className="me-2">
                    {contract ? 'Contract OK' : 'No Contract'}
                  </Badge>
                  {contract && (
                    <small className="text-muted">{formatAddress(contract.target || CONTRACT_ADDRESS || '')}</small>
                  )}
                </Navbar.Text>
