...
import { getAllBatches, getBatchInfo, formatAddress, formatTimestamp, getStatusText, getStatusBadgeClass, getSellerScore } from '../utils/blockchain-clean';
...
const Dashboard = () => {
  ...
  // Get all batches (public data) - allowed even without connected account
      ...
      const batchesWithDetails = await Promise.all(
        batches.slice(0, 10).map(async (batch) => {
          try {
            ...
            // fetch seller score for the processor if available
            let sellerScore = null;
            try {
              sellerScore = await getSellerScore(contract, batchInfo.processor);
            } catch (e) {
              console.warn('⚠️ [DASHBOARD] Could not fetch seller score for', batchInfo.processor, e);
            }
  ...
  return (
    <Container className="py-4">
      ...
      <Row className="mb-4">
        <Col>
          ...
          <p className="text-muted">
            Welcome to FoodTrace. Monitor your supply chain operations and track product batches.
          </p>
        </Col>
      </Row>
      ...
      <Row className="mb-4">
        ...
        <Col md={9}>
          {/* existing stats/cards will flow here */}
        </Col>
      </Row>
                    ...
                    <tbody>
                      {recentBatches.map((batch) => (
                        <tr key={batch.batchId}>
                          ...
                          <td>
                            {formatAddress(batch.currentOwner || batch.actor)}
                          </td>
                          <td>
                            {batch.sellerScore !== null && batch.sellerScore !== undefined ? (
                              <Badge bg={batch.sellerScore >= 75 ? 'success' : batch.sellerScore >= 40 ? 'warning' : 'secondary'}>
                                {batch.sellerScore}/100
                              </Badge>
                            ) : (
