...
import { getBatchDetails, updateTemperature, getSellerScore } from '../utils/blockchain-clean';
...
const BatchDetails = () => {
  ...
  const loadBatchDetails = useCallback(async () => {
    try {
      setLoading(true);
      // Validate route param 'id' before calling blockchain
      if (!id || isNaN(Number(id))) {
        throw new Error('Invalid batch id parameter');
      }
      const batchData = await getBatchDetails(contract, id);
      setBatch(batchData);
      // fetch seller score for the processor
      try {
        const score = await getSellerScore(contract, batchData.processor);
        setSellerScore(score);
      } catch (e) {
        console.warn('⚠️ [BATCH] Could not fetch seller score:', e);
      }
    } catch (error) {
      ...
      <Row>
              ...
              <Row>
                ...
                <Col md={6}>
                  ...
                  <p>
                    ...
                    {sellerScore !== null && sellerScore !== undefined ? (
                      <Badge bg={sellerScore >= 75 ? 'success' : sellerScore >= 40 ? 'warning' : 'secondary'}>
                        {sellerScore}/100
                      </Badge>
                    ) : (
        ...
        <Col lg={4}>
          ...
          {batch && batch.processor && (
            <ReputationCard sellerAddress={batch.processor} />
          )}

          {/* Buyer history for this batch (localStorage) */}
          {batch && (
            <BuyerHistoryCard batchId={batch.batchId || id} />
          )}
        </Col>
      </Row>
    </Container>
