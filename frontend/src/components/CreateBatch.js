    ...
    try {
      ...
      if (result.success) {
        if (result.batchId) {
          ...
          // Batch created but we couldn't determine the batchId from events/receipt
          console.warn('⚠️ [CREATE] Batch created but no ID available in result:', result);
          toast.success('Batch created successfully (ID unavailable). Transaction: ' + (result.transactionHash || 'n/a'));
          navigate('/');
        }
      } else {
